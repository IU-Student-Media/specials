...
function checkScroll() { // callback function 
    if(window.innerHeight > window.innerWidth) return; // no scroll begavior on mobile
    ...
    if (window.scrollY > startY) {
        ...
        if(navSections.classList.contains("show")) navSections.style.backgroundColor = 'black';
        slug.style.display = 'block';
    } else {
        nav.style.backgroundColor = 'rgba(0,0,0,1)';
        if(navSections.classList.contains("show")) navSections.style.backgroundColor = 'rgba(0,0,0,0.6)';
...
function toggleDropdown() {
    ...
    for(let i=0; i<navSections.childElementCount; ++i) navSections.children[i].classList.toggle("show");
}
