const data = {
    "url": "lil-bub-cat-legacy",
    "slug": "Lil BUB",
    "headline": "6 years after her death, Lil BUB's legacy lives on",
    "title": "Lil BUB",
    "pub_date": "Nov. 4, 2025",
    "bylines": {
        "By": {
            ...
            "email": "jsoco@iu.edu",
            // "twitter": "catcharron",
            "pfp": "https://s3.amazonaws.com/snwceomedia/ids/2c4e3172-bd2b-4343-ae6b-20119ce2ee37.original.jpg",
            "bio": "Nate Soco is a writer and podcast editor and has worked for the IDS since 2024."
        },
...
for (let type of byline_types) {
    if (data.bylines[type]) {
        ...
        if (!data.bylines[type].length) {
			let name = data.bylines[type].name == "Nate Soco" ? "Jonathan Soco" : data.bylines[type].name
            bylines_html += `<p>${type} <a href="https://idsnews.com/staff/${name.split(' ').join('-')}">${data.bylines[type].name}</a></p>`;
        } else {
            bylines_html += '<p>' + type;
        ...
        bios_html +=
            `<div class="bio">
                        <div>
                            ...
                            <div>
                            <p>${type} <a href="https://idsnews.com/staff/${name.split(' ').join('-')}" target="_blank">${author.name}</a></p>
