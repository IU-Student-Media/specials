const data = {
    "url": "election-guide-monroe-county-indiana-2022-state-federal-candidates",
    "slug": "Elections Guide State and Federal",
    "headline": "Meet the State and Federal Candidates",
    "title": "Elections Guide State and Federal",
    "pub_date": "Feb. 23, 2024",
    "bylines": {
        "By": {
            "name": "IDS Staff"
        },
...
for (let type of byline_types) {
    if (data.bylines[type]) {
        ...
        if (!data.bylines[type].length) {
            if (data.bylines[type].name == 'IDS Staff') {
...
/*
    Basically lines 92,93 and 96 caused the rest of the lines in this JS file not to run,
    which caused the social icons to not appear in the code. you can uncomment the lines
    below if you want to add back the bylines, bios or pub date. but it won't work if 
    there is no div for each of these bc it expects the document to have these elements.
*/
